 ...
 * @property {string} model - DeepSeek model to use (from environment variables)
...
class DeepSeekService extends EventEmitter {
     ...
     * Initializes DeepSeek client, loads tools from manifest, and sets up initial state.
     * 
    ...
    constructor() {
        ...
        this.loadedTools = {};
        logOut(`DeepSeekService`, `Loading tools...`);
        ...
        this.toolDefinitions.forEach((tool) => {
            ...
            logOut(`DeepSeekService`, `Loaded function: ${functionName}`);
        });
        logOut(`DeepSeekService`, `Loaded ${this.toolDefinitions.length} tools`);
    ...
    setCallParameters(to, from, callSid) {
        ...
        logOut(`DeepSeekService`, `Call to: ${this.twilioNumber} from: ${this.customerNumber} with call SID: ${this.callSid}`);
        this.messages.push({
            role: 'system',
    ...
    async executeToolCall(tool) {
        logOut(`DeepSeekService`, `Executing tool call with tool being: ${JSON.stringify(tool, null, 4)} `);

        try {
            let calledTool = this.loadedTools[tool.function.name];
            let calledToolArgs = JSON.parse(tool.function.arguments);
            logOut(`DeepSeekService`, `Executing tool call: ${tool.function.name} with args: ${JSON.stringify(calledToolArgs, null, 4)}`);
    ...
    async generateResponse(role = 'user', prompt) {
        ...
        logOut(`DeepSeekService`, `Generating response for ${role}: ${prompt}`);
        ...
        try {
            ...
            logOut(`DeepSeekService`, `Stream created`);
